...
keep_dt <- c("Chrom", "Pos", "Ref", "Alt", "Type", "Length",
...
overlap_phred40 <- process_caddscoring_dt(fread("data/GRCh38_CADDv1.6_whole_genome_SNVs_inclAnno.phred40.gnomad-exome_overlap.tsv.gz"),
                                    cols = keep_dt, 
                                    autosomes_only = TRUE) 

overlap_phred40 <- utils::type.convert(overlap_phred40, as.is = TRUE)
overlap_phred40_unique <- remove_duplicates_dt(overlap_phred40)
...
overlap_phred40_unique |> group_by(Chrom) |> count()
```
...
overlap_stopgain <- process_caddscoring_dt(fread("data/GRCh38_CADDv1.6_SNVs_STOP_GAINED.gnomad-exome_overlap.tsv.gz"),
                                     cols = keep, autosomes_only = TRUE) 

overlap_stopgain_unique <- remove_duplicates_dt(overlap_stopgain)
