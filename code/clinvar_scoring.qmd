...
## Custom functions
```{r}
# Remove duplicates based on Consequence Score and smallest relativ Position in gene  
remove_duplicates <- function(cadd, id = Ident, pos = relcDNApos) {
  cadd |> dplyr::group_by({{ id }}) |>
...
remaining_dups |> 
  ...
  count() |> arrange(desc(n)) |> kable()
  
remaining_dups |>
  filter(AnnoType == "CodingTranscript") |>
  select(Chrom, Pos, Ref, Alt, Consequence, GeneName, FeatureID, Exon, cDNApos, relcDNApos, protPos, relProtPos) |> kable()


remaining_dups |> 
  filter(AnnoType == "CodingTranscript") |> select(GeneName) |> unique() |> kable()

```

```{r}
#| tbl-cap: "Overview of remaining duplicates using ConScore and rel. Protein position"
#| tbl-subcap:
#|  -  "Number of remaining duplicates for each consequence" 
#|  -  "Duplicates in coding regions"
#|  -  "Genes with duplications that were not resolved" 
clinvar_scored_unique_prot <- remove_duplicates(clinvar_scored, pos = relProtPos)

remaining_dups <- clinvar_scored_unique_prot |> group_by(Ident) |> add_count() |>
  filter(n > 1) |> ungroup()
...
remaining_dups |>
  filter(AnnoType == "CodingTranscript") |>
  select(Ident, Consequence, GeneName, FeatureID, Exon, cDNApos, relcDNApos, protPos, relProtPos) |> kable()
...
`clinvar_scored_unique` only returns unique Ids: `r nrow(clinvar_scored_unique) == length(unique(clinvar_scored_unique$Ident))   `   


 `clinvar_scored_unique` has same amount of entries as previously determined number of unique variants: `r n_unique_variants == nrow(clinvar_scored_unique)`
